...
ADD EDAD nvarchar (6) check (EDAD in ('Adulto', 'Nino')) 
	;
...
CREATE TABLE Tipo_habitacion (
    tipo_habitacion_id INT PRIMARY KEY,
...
CREATE TABLE Area (
    ...
    FOREIGN KEY (id_hotel) REFERENCES Hotel.Hotels (hotel_id) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE Habitacion (
    habitacion_id INT PRIMARY KEY,
    numero_habitacion INT,
    estado NVARCHAR(15) CHECK (estado IN ('OCUPADO', 'LIBRE')),
    precio MONEY,
    precio_temporada_alta MONEY,
    precio_temporada_baja MONEY,
    caracteristicas NVARCHAR(200) NOT NULL,
    id_tipo_habitacion INT,
    id_area INT,
    FOREIGN KEY (id_tipo_habitacion) REFERENCES Tipo_habitacion (tipo_habitacion_id) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (id_area) REFERENCES Area (area_id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
select Hotel_Realta.dbo.Habitacion.numero_habitacion ,ABS(DATEDIFF(DAY, Hotel_Realta.dbo.Habitacion.fecha_inauguracion, MAX(Hotel_Realta.Booking.booking_order_detail.borde_checkout) )) AS dias_sumados 
...
group by Hotel_Realta.dbo.Habitacion.numero_habitacion,  
